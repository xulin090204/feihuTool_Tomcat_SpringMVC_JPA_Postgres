...
@Service
@Transactional
public class 除权数据ServiceImpl  extends 爸爸数据Service implements 除权数据Service{

	@Autowired
	除权信息Dao 除权信息dao;

	public 除权数据ServiceImpl() {
	...
	public String 下载除权导入数据() {
		...
		//s除权原始文件全路径[0]="D:\\FeihuTool_Convert\\除权数据.data1";
		...
		// 存储除权数据（解析原始文件、存入数据库）
		List<String> 除权数据list = InputDataUtil除权.getFileContext2(s除权原始文件全路径[0]);
